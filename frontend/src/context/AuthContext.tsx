...
export interface User { id: number; full_name: string; email: string; aadhaar_number?: string; city_ward?: string; avatar_url?: string; is_verified: boolean; created_at?: string; }
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
 const [user,setUser]=useState<User|null>(readUser); const [token,setToken]=useState<string|null>(()=>localStorage.getItem('jansetu_token')); const [isAuthModalOpen,setIsAuthModalOpen]=useState(false); const [authModalTab,setAuthModalTab]=useState<'login'|'register'>('login');
 const openAuthModal=(tab:'login'|'register'='login')=>{setAuthModalTab(tab);setIsAuthModalOpen(true)}; const closeAuthModal=()=>setIsAuthModalOpen(false);
 ...
 const logout=()=>{setToken(null);setUser(null);localStorage.removeItem('jansetu_token');localStorage.removeItem('jansetu_user');localStorage.removeItem('jansetu_admin_auth')}; const updateUser=(u:User)=>{setUser(u);localStorage.setItem('jansetu_user',JSON.stringify(u))};
